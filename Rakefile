def run_tests(platform, browser, version, junit_dir)
  system("SET platform=#{platform} && SET browserName=#{browser} && SET version=#{version} && SET JUNIT_DIR=#{junit_dir} && bundle exec parallel_cucumber features -n 20")
end
...
task :os_x_10_9_chrome_45 do
  run_tests('OS X 10.11', 'chrome', '45', 'junit_reports/os_x_10_9_chrome_45')
end
